...
class Block(pg.sprite.Sprite):
    ...
    def inflate(self, amount):
        ...
        self.image = pg.Surface(self.size)
        self.image.fill((255, 0, 0))  # Inflated block color (you can change it to whatever you like)
                        ...
                        all_sprites.add(absorber)
                          ...
                          for block in all_sprites.sprites():
                            if pg.sprite.collide_rect(absorber, block):
                                if block.width <= absorber.width**2 and block.height<= absorber.height **2:
                                           
                                    absorber.width += (int(block.width **0.2 ) + 1)
                                    absorber.height += (int(block.height **0.2 ) + 1)
                                    point +=1
