                ...
                if event.button == 1:  # Left mouse button
                    ...
                    if absorber is None:
                        ...
                        for block in all_sprites:
                            ...
                            absorber.image = pg.Surface((absorber.size[0], absorber.size[1]))
 
                            absorber.image.fill(absorber_color)
                            absorber.rect = pg.Rect(absorber.image.get_rect())
                            if block.rect.colliderect(absorber.rect):
                                 absorber.size += block.size
                                
                                absorber.image = pg.Surface((absorber.size[0], absorber.size[1]))
                                absorber.rect.inflate(5,5)
                                point += block.size[0] + block.size[1]
                                block.kill()
                            ...
                            elif absorber.size[0] > 1 and absorber.size[1] > 1 and not absorber.rect.colliderect(block.rect):
 
                                absorber.size = tuple([absorber.size[0] - 1, absorber.size[1] - 1])

                                absorber.image = pg.Surface((absorber.size[0], absorber.size[1]))
                                absorber.image.fill(absorber_color)
                                absorber.rect = absorber.image.get_rect(center=absorber.rect.center)
                            elif absorber.size[0] > 1 and absorber.size[1] > 1 and absorber.rect.colliderect(block.rect):
                                absorber.kill()  # Reset the absorber once it disappears
                                absorber = None
